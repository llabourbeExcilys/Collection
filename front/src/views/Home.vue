<template>
	<div>
		...
		<v-app-bar :clipped-left="clipped" fixed app>
			...
			<v-btn v-if="loggedIn" icon @click.stop="miniVariant = !miniVariant">
				<v-icon>mdi-{{ `chevron-${miniVariant ? 'right' : 'left'}` }}</v-icon>
			</v-btn>
			<v-toolbar-title v-text="title" />
			<v-spacer />
			<v-btn v-if="loggedIn" small color="primary" depressed @click="logout">
				Se déconnecter
			</v-btn>
		...
		<v-navigation-drawer v-if="loggedIn" v-model="rightDrawer" :right="right" temporary fixed>
			<!-- <v-list>
